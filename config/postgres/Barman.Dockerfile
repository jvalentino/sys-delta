...
# ARG BARMAN_VERSION=2.3-2.pgdg+1 can't find what the versions actually are
ARG DOCKERIZE_VERSION=v0.2.0
# grab gosu for easy step-down from root
# https://github.com/tianon/gosu/issues/17 keyserver doesn't work
...
RUN  wget -q https://www.postgresql.org/media/keys/ACCC4CF8.asc -O - | apt-key add - && \
     ...
     apt-get update && \
     apt-get install -y libffi-dev libssl-dev barman openssh-server

RUN  wget https://github.com/jwilder/dockerize/releases/download/$DOCKERIZE_VERSION/dockerize-linux-amd64-$DOCKERIZE_VERSION.tar.gz && \
     tar -C /usr/local/bin -xzvf dockerize-linux-amd64-$DOCKERIZE_VERSION.tar.gz

RUN apt-get -y install cron
ADD ./config/postgres/barman/crontab /etc/cron.d/barman
...
COPY ./config/postgres/ssh /home/postgres/.ssh
RUN chown -R postgres:postgres /home/postgres
COPY ./config/postgres/barman/configs/barman.conf /etc/barman.conf
COPY ./config/postgres/barman/configs/upstream.conf $UPSTREAM_CONFIG_FILE
COPY ./config/postgres/barman/bin /usr/local/bin/barman_docker
